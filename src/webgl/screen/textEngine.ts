...
export default function ScreenTextEngine(assists: Assists, sceneRTT: THREE.Scene) {
  ...
  const textBgMesh = new THREE.Mesh(new THREE.PlaneBufferGeometry(0, 0), new THREE.MeshBasicMaterial({ color: textColor }));
  rootGroup.add(textBgMesh);

  const caret = new THREE.Mesh(new THREE.PlaneBufferGeometry(h2Font.size, h2Font.size * 1.6), textMaterial);
  caret.position.z = -0.1;
  rootGroup.add(caret);

  let charUnderCaret: THREE.Mesh | undefined = undefined;
  function updateCharUnderCaret(isBlack: boolean) {
    if (charUnderCaret) (charUnderCaret.material as THREE.MeshBasicMaterial).color = new THREE.Color(isBlack ? "black" : textColor);
  }
  ...
  function generateGeometry(props: { str: string; font: FontInfo; highlight?: boolean; wrap?: boolean; isWord?: boolean; updateCharNextLoc?: boolean }): {
    ...
    if (props.highlight) {
      const background = new THREE.PlaneBufferGeometry(strLen + props.font.tracking * 2, props.font.height + props.font.leading / 2, 1, 1);

      background.translate(strLen / 2 - props.font.tracking / 2 + x, -props.font.height / 2 - y, -0.01);
  ...
  function mergeGeometriesWithMesh(baceMesh: THREE.Mesh, geometries: THREE.BufferGeometry[]) {
  ...
  function updateCharPos(inputBuffer: THREE.Mesh[] | TextGeometry[], helper: (obj: THREE.Mesh | TextGeometry, x: number, y: number) => void, shouldScroll: boolean) {
    const charWidth = h2Font.width + h2Font.tracking;
    const charsPerLine = Math.floor(screenWidth / charWidth);
    ...
    if (shouldScroll) {
      ...
      if (newNumberOfInputLines > oldNumberOfInputLines) scroll(newNumberOfInputLines - oldNumberOfInputLines, "lines");
  ...
  function userInput(change: Change, selectionPos: number) {
    if (change.type === "add") {
      if (change.loc === "end") {
        ...
        inputBuffer = [...inputBuffer.slice(0, change.loc), ...newChars, ...inputBuffer.slice(change.loc, inputBuffer.length)];
        updateCharPos(
          inputBuffer,
          (obj, x, y) => {
            obj = obj as THREE.Mesh;
      ...
      if (change.loc === "end") {
        ...
        const charsTODel = inputBuffer.slice(change.loc, change.loc + change.str.length);
        delChar(charsTODel);

        inputBuffer = [...inputBuffer.slice(0, change.loc), ...inputBuffer.slice(change.loc + change.str.length, inputBuffer.length)];
        updateCharPos(
          inputBuffer,
          (obj, x, y) => {
            obj = obj as THREE.Mesh;
  ...
  function placeImage(val: string) {
    ...
    if (Number.isNaN(aspectRatio)) throw new Error(`Error for image at: '${url}'. Image must have aspect ratio like this: '/path/to/image?aspect=1.5'`);
    const widthParam = parseFloat(params.get("width") ?? "");
    const width = Number.isNaN(widthParam) ? 1 : widthParam;

    const height = width / aspectRatio;

    const imageFrame = new THREE.Mesh(new THREE.PlaneBufferGeometry(width, height, 1, 1), new THREE.MeshBasicMaterial({ color: 0xff0000 }));
