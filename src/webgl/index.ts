...
export default function WebGL() {
  loadAssists((assists) => {
    ...
    const sizes = {
      ...
      portraitOffset: valMap(window.innerHeight / document.documentElement.clientWidth, [0.75, 1.75], [0, 2]),
    };

    // Scene
    const scene = new THREE.Scene();
    const ambientLight = new THREE.AmbientLight(0xff0000, 0.55);
    ...
    const tick = () => {
      ...
      camera.position.z = valMap(scroll, [0, 1], [-2.5 - sizes.portraitOffset, -10 - sizes.portraitOffset]);
      ...
      if (sizes.portraitOffset > 0.5) computerGroup.rotation.z = valMap(scroll, [0, 1], [-Math.PI / 2, 0]);
